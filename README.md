...
- **История измерений**: автоматическое сохранение в `C:/Users/<Пользователь>/Documents/SpeedtestNextGen/data/results.jsonl` и просмотр через `ui/history_interface.py` с автообновлением при переходе на вкладку.
- **Анонимный режим**: возможность не сохранять историю тестов для приватности.
...
fluent_speedtest/
  core/
    ...
    speedtest_service.py # общий сервис выбора движка и запуска тестов
...
- **Ключи**:
  ...
  - `anonymous_mode`: `true` или `false` — анонимный режим (не сохранять историю тестов).
...
- **Раздел «История»**:
  - **Автообновление**: история автоматически обновляется при переходе на вкладку.
...
- **Раздел «Настройки»**:
  ...
  - **«Анонимный режим»**: переключатель для отключения сохранения истории тестов (полезно для приватности).
...
- **Логирование**: системные настройки уровня и буфера логов находятся в `Настройки` (UI) и сохраняются в `settings.json` как `log_level` и `log_buffer_size`.
- **Полезные команды**:
...
- 📖 **[QUICK_START.md](docs/QUICK_START.md)** — быстрая шпаргалка (начните с этого!)
- 📚 **[GIT_WORKFLOW.md](docs/GIT_WORKFLOW.md)** — подробная инструкция по работе с ветками
...
Сборку нужно делать **перед релизом в main**:
1. Все функции протестированы в beta
2. Версия обновлена в `version.py`
3. Готовы к публикации релиза

### Как собрать?

Используйте готовые скрипты из папки `build_scripts/`:

```powershell
# Шаг 1: Убедитесь, что вы в ветке beta и статус "Beta"
git checkout beta
python set_version.py --show

# Шаг 2: Сборка в папку (для тестирования)
python build_scripts\build_folder.py
# Версия берётся автоматически из version.py

# Шаг 3: Протестировать собранный exe
build\SpeedTestNextgen.dist\SpeedTestNextgen.exe

# Шаг 4: Если всё работает - сборка для релиза (один файл)
python build_scripts\build_onefile.py
```

### Варианты сборки

**Standalone (папка)** - для разработки и тестирования:
```powershell
python build_scripts\build_folder.py
...
- ✅ **Автоматическая версия** из `version.py`
- ✅ **Автоматический компилятор**: Python 3.13+ → MSVC, ниже → MinGW-w64
- ✅ **Все пакеты включены**: PyQt5, QFluentWidgets (транзитивно тянет qframelesswindow), openpyxl, speedtest-cli
- ✅ **Иконка**: берётся из `assets/app.ico`
